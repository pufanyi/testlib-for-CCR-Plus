...
#ifndef __BORLANDC__
	return ((ra != ra) == true) && ((ra == ra) == false) && ((1.0 > ra) == false)
	       && ((1.0 < ra) == false);
#else
...
class random_t {
	...
	public:
		...
		double next() {
			return (double)(((long long)(nextBits(26)) << 27) + nextBits(27)) / (double)(
			           1LL << 53);
		}
...
class BufferedFileInputStreamReader: public InputStreamReader {
	...
	public:
		BufferedFileInputStreamReader(std::FILE* file,
		                              const std::string& name): file(file), name(name) {
...
NORETURN void InStream::quit(TResult result, const char* msg) {
	...
	if (resultName != "") {
		...
		if (result == _ok) std::fprintf(resultFile, "1\n%s", msg);
		else if (result == _points) std::fprintf(resultFile, "%s", msg);
		else std::fprintf(resultFile, "0\n%s", msg);
...
static inline double stringToStrictDouble(InStream& in, const char* buffer,
	...
	if (minAfterPointDigitCount < 0)
		in.quit(_fail,
		        "stringToStrictDouble: minAfterPointDigitCount should be non-negative.");
...
long long InStream::readLong(long long minv, long long maxv,
	...
	if (result < minv || result > maxv) {
		if (variableName.empty())
			quit(_wa, ("0\nInteger " + vtos(result) + " violates the range [" + vtos(
			               minv) + ", " + vtos(maxv) + "]").c_str());
		else
			quit(_wa, ("0\nInteger parameter [name=" + variableName + "] equals to " + vtos(
			               result) + ", violates the range [" + vtos(minv) + ", " + vtos(
			               maxv) + "]").c_str());
	}
	...
	if (result < minv || result > maxv) {
		if (variableName.empty())
			...
			quit(_wa, ("0\nInteger parameter [name=" + std::string(variableName) +
			           "] equals to " + vtos(result) + ", violates the range [" + vtos(
			               minv) + ", " + vtos(maxv) + "]").c_str());
	}
...
double InStream::readReal(double minv, double maxv,
	...
	if (result < minv || result > maxv) {
		if (variableName.empty())
			quit(_wa, ("0\nDouble " + vtos(result) + " violates the range [" + vtos(
			               minv) + ", " + vtos(maxv) + "]").c_str());
		else
			quit(_wa, ("0\nDouble parameter [name=" + variableName + "] equals to " + vtos(
			               result) + ", violates the range [" + vtos(minv) + ", " + vtos(
			               maxv) + "]").c_str());
	}
...
double InStream::readStrictReal(double minv, double maxv,
	...
	if (result < minv || result > maxv) {
		if (variableName.empty())
			quit(_wa, ("0\nStrict double " + vtos(result) + " violates the range [" + vtos(
			               minv) + ", " + vtos(maxv) + "]").c_str());
		else
			quit(_wa, ("0\nStrict double parameter [name=" + variableName + "] equals to " +
			           vtos(result) + ", violates the range [" + vtos(minv) + ", " + vtos(
			               maxv) + "]").c_str());
	}
...
void InStream::readStringTo(std::string& result, const pattern& p,
	...
	if (!p.matches(result)) {
		if (variableName.empty())
			...
			quit(_wa, ("0\nLine [name=" + variableName + "] equals to \"" + __testlib_part(
			               result) + "\", doesn't correspond to pattern \"" + p.src() + "\"").c_str());
	}
}
...
static void __testlib_ensuresPreconditions() {
	...
	if (!__testlib_isNaN(+__testlib_nan()))
		quit(_fail,
		     "Function __testlib_isNaN is not working correctly: possible reason is '-ffast-math'");
	if (!__testlib_isNaN(-__testlib_nan()))
		quit(_fail,
		     "Function __testlib_isNaN is not working correctly: possible reason is '-ffast-math'");
}
...
void registerInteraction(int argc, char* argv[]) {
	...
	if (strcmp(argv[2], "stdout") != 0) {
		...
		if (tout.fail() || !tout.is_open())
			quit(_fail, std::string("Can not write to the test-output-file '") + argv[2] +
			     std::string("'"));
	}
...
void registerLocalChecker(const std::string &_inputFile,
                          const std::string &_outputFile, const std::string &_answerFile,
                          int _perfectScore, const std::string &_scoreFile,
...
void registerLemonChecker(int argc, char* argv[]) {
	...
	registerLocalChecker(argv[1], argv[2], argv[3], atoi(argv[4]), argv[5],
	                     argv[6]);
}
...
NORETURN void __testlib_expectedButFound(TResult result, double expected,
	...
	std::string expectedString = removeDoubleTrailingZeroes(format("%.12f",
	                             expected));
...
NORETURN void expectedButFound<std::string>(TResult result,
        std::string expected, std::string found, const char* prependFormat, ...) {
	FMT_TO_RESULT(prependFormat, prependFormat, prepend);
...
NORETURN void expectedButFound<const char*>(TResult result,
        const char* expected, const char* found, const char* prependFormat, ...) {
	FMT_TO_RESULT(prependFormat, prependFormat, prepend);
...
NORETURN void expectedButFound<long double>(TResult result,
        long double expected, long double found, const char* prependFormat, ...) {
	FMT_TO_RESULT(prependFormat, prependFormat, prepend);
	__testlib_expectedButFound(result, double(expected), double(found),
	                           prepend.c_str());
}
